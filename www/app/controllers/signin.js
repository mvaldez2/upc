...
define([
  ...
  app.controller('SigninCtrl', [
    ...
    function ($scope, $stateParams, $window, $ionicPopup, eventService, $firebaseArray, $ionicHistory, Calendar, GAPI, $state, $rootScope) {
      ...
      var cal = $firebaseArray(calendarRef);
      $scope.cal = $firebaseArray(calendarRef);
      // stops working after the first time. It's getting an undefined event afterwards
      $scope.deleteOldEvent = function () {
        for (var i in cal) {
          var eventDate;
          if (cal[i].start.dateTime == undefined) {
            console.log("weird event ", cal[i].summary)
            eventDate = new Date(cal[i].start.date);
            var end = new Date(cal[i].end.date);
            if (eventDate <= today) {
              ref.child("calendar/" + cal[i].id).remove();
              console.log("Removed: ", cal[i].summary)
            }
          } else {
